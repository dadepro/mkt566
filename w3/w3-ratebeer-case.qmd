...
Last week you explored a clean, simulated dataset one variable at a time. This week the data is **real, big, and dirty**: 292,680 beer reviews from RateBeer (a beer-review website that has since closed), and the questions are about **covariation**, how variables move together.
...
4. **Give your assistant a memory** (2 minutes, recommended). Every tool reads one file at the start of each chat: `CLAUDE.md` for Claude Code, `AGENTS.md` for Codex, `.github/copilot-instructions.md` for Copilot, `.cursor/rules` for Cursor. Put your standing instructions there once, and you never retype them. Paste this into your assistant:

   > Create the instructions file your tool reads at the start of every chat in this folder (CLAUDE.md for Claude Code, AGENTS.md for Codex, .github/copilot-instructions.md for Copilot, .cursor/rules for Cursor) with these rules: I am a beginner. Use R with data.table and ggplot2. Add a short comment above each step. Do not run code; I run it myself and paste the output. Every chart must be printed in the R Markdown chunk, not only saved with ggsave. After every request I make, append one line describing it to ai-log.md in this folder, and note anything you got wrong that we fixed.

   Open the file it created and read it: it is plain text, and you can edit it any time. From now on the AI log writes itself as you work. (Copilot only follows this file in agent mode; if you use Copilot chat, keep using the log prompt at the end.)
5. Everything else (the R extension, rmarkdown, pandoc) is what you set up for the variation case. If knitting failed for you last week, fix it now, before class, with the [variation case setup steps](https://raw.githack.com/dadepro/mkt566/main/w2/w2-1-variation-case.html#setup-5-minutes) or in office hours.
...
`ai-log.md` accumulates across chats, but each chat only knows its own conversation. If you set up the memory file in Setup step 4, the assistant logs each request as you go: check `ai-log.md` once in a while to confirm. Otherwise, **before you close each chat, make sure the assistant has saved that session's log to `ai-log.md`**, or that session's requests are lost. The prompt, same as last week:
...
If you saved it as a reusable command in week 1, it is one keystroke.
:::
...
**Explain it back:** what does `str()` do?
...
> Convert ratebeer-case.R into ratebeer-report.Rmd in this folder: an R Markdown report titled "The RateBeer Case", output format html_document. Right under the title, put my first name, last name, and 10-digit USC ID: ______ . One section per task, each containing the code chunk for that task, followed by three placeholders: "**My prediction:**", "**My answer:**", and "**Explain it back:**". Every chunk must display its chart in the document (print the ggplot object; do not only save it with ggsave, and do not insert the PNG files from the figures folder), so that the figures are embedded in the knitted HTML. Use fread with the file name ratebeer.csv so it knits from this folder.
...
Open the knitted `ratebeer-report.html` and check that **every chart is visible inside it**. If a section shows code but no chart, the chunk only saved the figure to a file; tell the assistant to print the chart in the chunk. The HTML must be self-contained, because the grader only receives that one file, not your figures folder.
:::
...
**Submit:** `ratebeer-report.html` and `ai-log.md`, on **Brightspace**, by **Sunday, Sept. 20, 11:59 pm**. We work on this case over two sessions: Tasks 1–6 on Thursday, Sept. 10, and Tasks 7–9 plus a debrief on Tuesday, Sept. 15. Every student submits their own report.
