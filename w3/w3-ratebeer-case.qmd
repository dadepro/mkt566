...
4. **Give your assistant a memory** (2 minutes, recommended). Every tool reads one file at the start of each chat: `CLAUDE.md` for Claude Code, `AGENTS.md` for Codex, `.github/copilot-instructions.md` for Copilot, `.cursor/rules` for Cursor. Put your standing instructions there once, and you never retype them. Paste this into your assistant:
   ...
   > Create the instructions file your tool reads at the start of every chat in this folder (CLAUDE.md for Claude Code, AGENTS.md for Codex, .github/copilot-instructions.md for Copilot, .cursor/rules for Cursor) with these rules: I am a beginner who has never coded. Use R with data.table and ggplot2. Add a short comment above each step. Do not run code; I run it myself and paste the output. Every chart must be printed in the R Markdown chunk, not only saved with ggsave. After every request I make, append one line describing it to ai-log.md in this folder, and note anything you got wrong that we fixed.
