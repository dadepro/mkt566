...
`ai-log.md` accumulates across chats, but each chat only knows its own conversation. So **before you close each chat, make sure the assistant has saved that session's log to `ai-log.md`**, or that session's requests are lost. The prompt, same as last week:
