...
2. **`w2-1-simulate-marketing-dataset.R`**: generates Thursday's case dataset,
...
::: {.smaller-80 .muted}
Thursday's in-class case is a separate handout, and it has **no starter code**
...
::: {.smaller-90}
Already informative: no missing values, `facebook` spend starts at **zero**
(some campaigns skip it), `youtube` gets by far the largest budgets, and
`sales` vary by a factor of ~17.
:::
...
sales: only three bars, almost no information.
:::
...
::: {.smaller-80 .muted}
Now we can see the shape: most weeks sell 10–20, with a right tail of strong
weeks.
:::
...
::: {.smaller-80 .muted}
Same variable type, three different shapes: YouTube spend is spread almost
uniformly, Facebook is flat-ish with a low mode, newspaper is **right-skewed**:
a few campaigns spend a lot.
:::
...
::: {.smaller-80 .muted}
Newspaper shows flagged outliers, but the three channels live on very
different scales, so the small ones are squashed. Sound familiar?
:::
...
::: {.smaller-80 .muted}
On the log scale the comparison is fair, and a different picture appears:
now **low-spend** outliers become visible too.
:::
...
# Thursday: The Variation Case {.divider}

## The variation case

In class on Thursday you will apply all of this to a fresh dataset:
...
whole analysis, exactly as discussed on Tuesday: you describe the chart, your
...
::: {.box}
Bring your laptop Thursday, with the [week 1
setup](https://raw.githack.com/dadepro/mkt566/main/w1/vscode-setup.html)
working: VS Code, an AI assistant, and R. That is all you need.
:::
...
- [**The variation case handout**](https://raw.githack.com/dadepro/mkt566/main/w2/w2-1-variation-case.html):
  Thursday's vibecoding exercise (bring your laptop!)
:::
