...
- What does the fact the median is larger than the mean tell you?
- What marketing strategy might create this shape?

Answers:

- Distribution is right-skewed (long right tail).
- Mean is larger than median means that there are large outliers (mean is sensitive to outliers).
- Likely caused by a few very large campaigns vs. many small ones.
...
- Log reveals a more symmetric distribution because it compresses large values, reducing the skewness in right-skewed data and making it more symmetric
- Typical spend looks clearer, clustering around mid-range.
- Log scale reduces impact of extreme outliers.
...
- Email is underrepresented.
- Search and Social dominate volume.
- Non digital channels/more expensive are used less frequently which is generally common in today's marketing.
...
- There are several outliers beyond the upper fence.
- Outliers are all large spend values.
- Depends on analysis goals; could distort averages. Logging them or using robust stats may help.
...
Answers:
- Log scale compresses large values and expand small ones, making it easier to see smaller outliers.
