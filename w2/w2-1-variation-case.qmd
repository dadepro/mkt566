...
Every deliverable in this course ships with an `ai-log.md`. You do not write it; the assistant does. The file accumulates across chats, but each chat only knows its own conversation, so you must ask **before closing each chat**, or that session's requests are lost. The last step of this case is pasting the log prompt from Tuesday's slides:
