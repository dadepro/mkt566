...
Second, a one-time fix. Recent versions of R (4.5 and newer, which is what you just installed) changed how R starts up, and the R extension has a known bug with it ([issue #1696](https://github.com/REditorSupport/vscode-R/issues/1696)): without the fix, charts still open in a separate window. Copy the block below, paste it into the R console from Step 3 (the panel ending in `>`), and press Enter:
